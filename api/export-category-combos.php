<?php
/**
 * API Module: Export CategoryCombos with Dependencies
 * Endpoint pour exporter des CategoryCombos avec toutes leurs dépendances
 */
...
/**
 * Classe pour gérer l'export des CategoryCombos avec dépendances
 */
class CategoryComboExportAPI
{
    private $dhis2Url;
    private $dhis2Auth;
    ...
    public function handleRequest()
    {
        try {
            ...
            $categoryComboUids = $input['categoryComboUids'] ?? '';
            $includeGroups = $input['includeGroups'] ?? true;
            $includeGroupSets = $input['includeGroupSets'] ?? true;

            if (empty($categoryComboUids)) {
                throw new Exception('categoryComboUids est requis');
            }

            // Parser les UID
            $rawUids = preg_split('/[,;]/', $categoryComboUids);
            ...
            error_log("📦 Export de " . count($uniqueUids) . " CategoryCombo(s): " . implode(', ', $uniqueUids));

            $allComboExports = [];

            // Récupérer chaque CategoryCombo
            foreach ($uniqueUids as $uid) {
                try {
                    $comboExport = $this->exportCategoryCombo($uid, $includeGroups, $includeGroupSets);
                    $allComboExports[] = $comboExport;
                } catch (Exception $e) {
                    error_log("❌ Erreur CategoryCombo {$uid}: " . $e->getMessage());
                    continue;
                }
            }

            if (empty($allComboExports)) {
                throw new Exception('Aucun CategoryCombo valide trouvé.');
            }
            ...
            error_log('✅ Export multi-CategoryCombo terminé ! ' . json_encode($summary));

            $this->sendResponse(true, "Export de {$summary['categoryCombos']} CategoryCombo(s) réussi", [
                'categoryComboCount' => $summary['categoryCombos'],
                'data' => $exportPackage,
                'summary' => $summary
            ]);
...
// Point d'entrée
try {
    $api = new CategoryComboExportAPI();
    $api->handleRequest();
} catch (Exception $e) {
