...
const FIELD_DEFINITIONS = {
    users: [
        { id: 'userCredentials.username', name: 'Nom d\'utilisateur (Requis)', required: true },
        { id: 'firstName', name: 'Prénom (Requis)', required: true },
        { id: 'surname', name: 'Nom (Requis)', required: true },
        { id: 'email', name: 'Email', required: false },
        { id: 'phoneNumber', name: 'Téléphone', required: false },
        { id: 'userCredentials.password', name: 'Mot de Passe', required: false },
        { id: 'userCredentials.userRoles', name: 'Rôles (séparés par virgule)', required: false },
        { id: 'userGroups', name: 'Groupes (séparés par virgule)', required: false },
        { id: 'organisationUnits', name: 'Unités Org (séparés par virgule)', required: false },
        { id: 'id', name: 'UID (Mise à jour)', required: false }
    ]
};

// ... (rest of the file until processExcelData)
...
function processExcelData() {
    ...
    rows.forEach(row => {
        ...
        Object.keys(appState.mapping).forEach(colIndex => {
            const fieldPath = appState.mapping[colIndex];
            const val = row[colIndex];

            if (val !== undefined && val !== null && val !== '') {
                // Handle dot notation for nesting (e.g. userCredentials.username)
                ...
                parts.forEach((part, index) => {
                    if (index === parts.length - 1) {
                        // Leaf
                        current[part] = processValue(val, fieldPath);
                    } else {
                        // Node
...
function processValue(val, field) {
    // Helper to process specific fields (comma separated lists to arrays)
    if (['userCredentials.userRoles', 'userGroups', 'organisationUnits'].includes(field)) {
        if (typeof val === 'string') {
            // Expecting formatting like "UID1,UID2" or "[UID1],[UID2]"
            // Clean up and mapping to objects if needed
            // DHIS2 expects [{id: 'UID1'}, {id: 'UID2'}] for collections
